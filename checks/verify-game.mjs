...
assert([...collected].every(n=>Number.isInteger(n)&&n>=0&&n<32),'Save normalizes star ids');
assert([...visited].every(n=>Number.isInteger(n)&&n>=0&&n<4),'Save normalizes neighborhood ids');
...
assert(stars.length===32,'All original stars preserved');assert(crates.length===3,'All gadgets preserved');
...
reset(95,0);advanceFrame(1/60);assert(player.position.distanceTo(spawn)<.01,'Boundary rescue');
...
`;
try { await import('data:text/javascript;base64,'+Buffer.from(source.replace("import * as T from './three.module.js';",importLine)+tests).toString('base64')); } catch(e) { console.error('CHECK FAILED:',e.message); process.exit(1); }
